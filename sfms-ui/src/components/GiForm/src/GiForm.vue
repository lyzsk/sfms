...
props.columns.forEach((item) => {
    if (item.request && typeof item.request === "function" && item?.init) {
        item.request(props.modelValue).then((res) => {
            dicData[item.field] = item.resultFormat
                ? item.resultFormat(res)
                : res.data;
        });
    }
});
            ...
            arr.forEach((i) => {
                if (i.request && Boolean(newVal[item.field])) {
                    i.request(props.modelValue).then((res) => {
                        dicData[i.field] = i.resultFormat
                            ? i.resultFormat(res)
                            : res.data;
                        if (
                            !dicData[i.field]
                                .map((i: any) => i.value)
                                .includes(props.modelValue[i.field])
                        ) {
                            emit(
                                "update:modelValue",
                    ...
                    emit(
                        "update:modelValue",
                        Object.assign(props.modelValue, { [i.field]: "" })
                    );
                }
            });
        }
