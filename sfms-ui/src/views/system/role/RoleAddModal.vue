    ...
    >
        ...
        >
                ...
                >
                    <a-space>
                        <a-checkbox
                            ...
                            @change="onExpanded('menu')"
                            >展开/折叠</a-checkbox
                        >
                        <a-checkbox
                            v-model="isMenuCheckAll"
                            @change="onCheckAll('menu')"
                            >全选/全不选</a-checkbox
                        >
                        <a-checkbox v-model="form.menuCheckStrictly"
                            >父子联动</a-checkbox
                        >
                    </a-space>
                    <template #extra>
            ...
            <fieldset v-show="current === 3">
                ...
                >
                    <a-space>
                        <a-checkbox
                            v-model="isDeptExpanded"
                            @change="onExpanded('dept')"
                            >展开/折叠</a-checkbox
                        >
                        <a-checkbox
                            v-model="isDeptCheckAll"
                            @change="onCheckAll('dept')"
                            >全选/全不选</a-checkbox
                        >
                        <a-checkbox v-model="form.deptCheckStrictly"
                            >父子联动</a-checkbox
                        >
                    </a-space>
                    <template #extra>
        ...
        <template #footer>
            <a-space size="large">
                ...
                <a-button v-if="current === 3" type="primary" @click="onClickOk"
                    >确定</a-button
                >
            </a-space>
        </template>
...
const onNext = async () => {
    try {
        if (current.value === 1) {
            const isInvalid = await formRef.value?.validateField([
                "name",
                "code",
                "sort",
                "description",
...
const handleTreeAction = (type, action) => {
    ...
    if (ref && action === "expand") {
        ref.value?.expandAll(
            type === "menu" ? isMenuExpanded.value : isDeptExpanded.value
        );
    } else if (ref && action === "check") {
        ref.value?.checkAll(
            type === "menu" ? isMenuCheckAll.value : isDeptCheckAll.value
        );
    }
};
