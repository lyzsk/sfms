...
import Viewer, { type Animate } from "@/views/virtualfab/modules/Viewer";
import Floors from "@/views/virtualfab/modules/Floors";
import ModelLoader from "@/views/virtualfab/modules/ModelLoder";
import * as THREE from "three";
import gsap from "gsap";
import Event from "@/views/virtualfab/modules/Viewer/Events";
import BoxHelperWrap from "@/views/virtualfab/modules/BoxHelperWrap";
import { checkNameIncludes, findParent } from "@/views/virtualfab/utils";
        ...
        if (!isModelSelectName && oldOffice) {
            ...
            office.object
                ...
                .traverse(function (child: {
                    ...
                    if (child.isMesh) {
                        child.material = oldmodel.getObjectByName(
                            child.name
                        ).material;
                    }
                });
        ...
        child.children.forEach((Mesh: any) => {
            if (child.name === modelSelectName) {
                child.children.forEach((Mesh: { material: any; name: any }) => {
                    Mesh.material = oldmodel.getObjectByName(
                        Mesh.name
                    ).material;
                });
            } else {
