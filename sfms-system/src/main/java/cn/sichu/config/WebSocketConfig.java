                ...
                if (accessor != null) {
                    if (StompCommand.CONNECT.equals(accessor.getCommand())) {
                        ...
                        if (StrUtil.isNotBlank(bearerToken) && bearerToken.startsWith("Bearer ")) {
                            bearerToken =
                                bearerToken.substring(SecurityConstants.JWT_TOKEN_PREFIX.length());
                            String username = JWTUtil.parseToken(bearerToken).getPayloads()
                                .getStr(JWTPayload.SUBJECT);
                            if (StrUtil.isNotBlank(username)) {
                                accessor.setUser(() -> username);
                                eventPublisher.publishEvent(
                                    new UserConnectionEvent(this, username, true));
                            }
                        }
                        ...
                        if (accessor.getUser() != null) {
                            ...
                            eventPublisher.publishEvent(
                                new UserConnectionEvent(this, username, false));
                        }
                    }
                }
