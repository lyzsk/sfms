...
import cn.sichu.core.utils.expression.ExpressionUtils;
import cn.sichu.security.limiter.annotation.RateLimiter;
import cn.sichu.security.limiter.annotation.RateLimiters;
import cn.sichu.security.limiter.autoconfigure.RateLimiterProperties;
import cn.sichu.security.limiter.enums.LimitType;
import cn.sichu.security.limiter.exception.RateLimiterException;
...
public class RateLimiterAspect {
    ...
    public RateLimiterAspect(RateLimiterProperties properties,
        RateLimiterNameGenerator nameGenerator, RedissonClient redissonClient) {
    ...
    private boolean isRateLimited(ProceedingJoinPoint joinPoint, RateLimiter rateLimiter) {
        try {
            ...
            RRateLimiter rRateLimiter = RATE_LIMITER_CACHE.computeIfAbsent(cacheKey,
                key -> redissonClient.getRateLimiter(cacheKey));
            // 限流器配置
            RateType rateType =
     ...
     * @return 是否需要更新配置
     */
    private boolean isConfigurationUpdateNeeded(RRateLimiter rRateLimiter, RateType rateType,
        long rate, long rateInterval, RateIntervalUnit rateIntervalUnit) {
        ...
        return !Objects.equals(config.getRateType(), rateType) || !Objects.equals(config.getRate(),
            rate) || !Objects.equals(config.getRateInterval(),
